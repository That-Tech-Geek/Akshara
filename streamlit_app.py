...
# Add unique keys for each widget to avoid duplicate element IDs
savings_goal_desc = st.text_input(
    translate_text("Enter your savings goal (e.g., Buy a cow, open a shop)", selected_lang),
    key="savings_goal_desc"
)
savings_goal_amount = st.number_input(
    translate_text("Enter the total savings goal amount (INR)", selected_lang),
    min_value=100,
    key="savings_goal_amount"
)
duration = st.number_input(
    translate_text("How many months to save?", selected_lang),
    min_value=1,
    max_value=24,
    key="duration"
)
monthly_savings = st.number_input(
    translate_text("Enter monthly saving amount (INR)", selected_lang),
    min_value=100,
    key="monthly_savings"
)

if st.button(translate_text("Create Savings Plan", selected_lang), key="create_savings_plan"):
